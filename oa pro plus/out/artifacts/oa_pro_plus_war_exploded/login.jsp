...
<div id="container">
    <div id="main">
        ...
        <form action="${pageContext.request.contextPath}/user/login" method="post" onsubmit="
        if (this.elements[0].value.length === 0 && this.elements[1].value.length === 0) {alert('请输入用户名和密码');    return false;}
    else if (this.elements[0].value.length === 0) {alert('请输入用户名'); return false;}
    else if (this.elements[1].value.length === 0) {alert('请输入密码');  return false;}
    else if (!document.getElementById('agreement').checked) {alert('请先勾选同意协议'); return false;}">
            <label class="label">
